...
def get_sorted_spaced_maxes(data, n: int, delta: int):
    ...
    x_days_before_next = maxes["t"].shift(-1).isna() | (maxes["t"].shift(-1) - maxes["t"] > pd.Timedelta(days=delta))
    x_days_after_prev = maxes["t"].shift(1).isna() | (maxes["t"] - maxes["t"].shift(1) > pd.Timedelta(days=delta))
    ...
    spaced_maxes = maxes[(greater_than_next | x_days_before_next) & (greater_than_prev | x_days_after_prev)].reset_index(drop=True)
