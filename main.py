...
from utils.data import (
    fetch_data_from_api,
    get_sorted_spaced_maxes,
    generate_daily_params,
    generate_hourly_params,
)
...
def main(page: ft.Page):
    ...
    start_date_input = ft.TextField(
        label="Start date (YYYY-MM-DD)", value="1900-01-01", width=200
    )
    ...
    nb_floods_input = ft.TextField(label="Number of floods", value=5, width=160)
    ...
    def refresh_list_view():
        list_view.controls = [
            list_view_item(date, item["color"], item["max"])
            for date, item in store.data.items()
        ]
        list_view.update()
        ...
        try:
            plt.savefig(
                str(
                    Path.home()
                    / "Downloads"
                    / f"{station_code_input.value}_{start_date_input.value}_{end_date_input.value}.svg"
                )
            )
            notify(
                page,
                "Successfully saved, check your Downloads folder.",
                ft.colors.GREEN_ACCENT_700,
            )
        except:
            notify(
                page,
                "Something wrong happened while exporting your graph to SVG.",
                ft.colors.ERROR,
            )
    ...
    def export_to_excel(e):
        try:
            filepath = str(
                Path.home()
                / "Downloads"
                / f"{station_code_input.value}_{start_date_input.value}_{end_date_input.value}.xlsx"
            )
            ...
            notify(
                page,
                "Something wrong happened while exporting your data to Excel.",
                ft.colors.ERROR,
            )
    ...
    export_to_excel_button = ft.FilledButton(
    ...
    export_to_svg_button = ft.FilledButton(
        "Export graph to SVG",
        icon="image",
        height=30,
        on_click=export_to_svg,
        disabled=True,
    )

    actions_container = ft.Container(
        content=ft.Row(
            controls=[export_to_excel_button, export_to_svg_button],
    ...
    def search(e):
        ...
        data, err = fetch_data_from_api(
            station_code_input.value,
            generate_daily_params(
                pd.Timestamp(start_date_input.value), pd.Timestamp(end_date_input.value)
            ),
        )
        if err != None:
            notify(
                page,
                "Something wrong happened while fetching the data from the API.",
                ft.colors.ERROR,
            )
            return

        sorted_spaced_maxes = get_sorted_spaced_maxes(
            data, int(nb_floods_input.value), delta
        )
        ...
        for _, row in sorted_spaced_maxes.iterrows():
            peak_date = row["t"]
            start_date, end_date = peak_date - pd.Timedelta(
                days=delta
            ), peak_date + pd.Timedelta(days=delta)
            data, err = fetch_data_from_api(
                station_code_input.value, generate_hourly_params(start_date, end_date)
            )
            if err != None:
                notify(
                    page,
                    f"Could not fetch hourly data for flood {peak_date}",
            ...
            chro_df["t_norm"] = (
                chro_df["t"] - chro_df.loc[chro_df["v"].idxmax()]["t"]
            ...
            ft.Container(
                content=ft.Row(
                    controls=[
                        ...
                        nb_floods_input,
                        ft.FilledButton(
                            "Search", icon="search", height=50, on_click=search
                        ),
                    ],
                    alignment=ft.MainAxisAlignment.CENTER,
                ),
